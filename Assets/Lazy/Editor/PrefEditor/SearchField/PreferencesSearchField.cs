    ...
    public class PreferencesSearchField : UnityEditor.IMGUI.Controls.SearchField
    {
        ...
        )
        {
            ...
            if (
                ...
                && contextMenuRect.Contains(Event.current.mousePosition)
            )
            {
                void OnDropdownSelection(object parameter)
                {
                    SearchMode = (SearchModePreferencesEditorWindow)
                        Enum.Parse(typeof(SearchModePreferencesEditorWindow), parameter.ToString());
                    this.OnDropdownSelection?.Invoke();
                }

                GenericMenu menu = new();
                foreach (
                    SearchModePreferencesEditorWindow item in Enum.GetValues(
            ...
            var contextMenuOverlayStyle = GUIStyle.none;
            contextMenuOverlayStyle.contentOffset = new Vector2(9, 5);
            GUI.Box(
                new Rect(rect.x, rect.y, 5, 5),
                EditorGUIUtility.IconContent("d_ProfilerTimelineDigDownArrow@2x"),
            ...
            if (!HasFocus() && string.IsNullOrEmpty(text))
            {
                GUI.enabled = false;
                GUI.Label(
                    new Rect(rect.x + 14, rect.y, 40, rect.height),
                    Enum.GetName(typeof(SearchModePreferencesEditorWindow), SearchMode)
                );
                GUI.enabled = true;
            }

            contextMenuOverlayStyle.contentOffset = new Vector2();
            return result;
        }
        ...
        public new string OnToolbarGUI(Rect rect, string text)
        {
            return OnGUI(
                rect,
                text,
