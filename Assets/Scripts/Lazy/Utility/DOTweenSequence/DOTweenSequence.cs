    ...
    public class DOTweenSequence : MonoBehaviour
    {
        ...
        private Tween CreateTween(bool reverse = false)
        {
                ...
                for (int i = m_Sequence.Length - 1; i >= 0; i--)
                {
                    ...
                    if (tweener == null)
                    {
                        Debug.LogErrorFormat(
                            ...
                            i,
                            item.AnimationType,
                            item.Target == null ? "null" : item.Target.GetType().Name
                        );
                        continue;
                    }
                    switch (item.AddType)
            ...
            sequence
                ...
                .SetDelay(m_Delay);
            if (m_OnPlay != null)
                sequence.OnPlay(m_OnPlay.Invoke);
            if (m_OnUpdate != null)
            ...
            public Tween CreateTween(bool reverse)
            {
                ...
                switch (AnimationType)
                {
                    case DOTweenType.DOMove:
                        {
                            ...
                            Vector3 targetValue = UseToTarget
                                ? ((Transform)ToTarget).position
                                : ToValue;
                    ...
                    case DOTweenType.DOMoveX:
                        {
                            var transform = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).position.x
                                : ToValue.x;
                    ...
                    case DOTweenType.DOLocalMove:
                        {
                            var transform = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).localPosition
                                : (Vector3)ToValue;
                            var startValue = UseFromValue
                                ? (Vector3)FromValue
                    ...
                    case DOTweenType.DOLocalMoveX:
                        {
                            var transform = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).localPosition.x
                                : ToValue.x;
                    ...
                    case DOTweenType.DOLocalMoveY:
                        {
                            var transform = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).localPosition.y
                                : ToValue.x;
                    ...
                    case DOTweenType.DOLocalMoveZ:
                        {
                            var transform = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).localPosition.z
                                : ToValue.x;
                    ...
                    case DOTweenType.DOScaleX:
                        {
                            var com = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).localScale.x
                                : ToValue.x;
                    ...
                    case DOTweenType.DOScaleY:
                        {
                            var com = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).localScale.y
                                : ToValue.x;
                    ...
                    case DOTweenType.DOScaleZ:
                        {
                            var com = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).localScale.z
                                : ToValue.x;
                    ...
                    case DOTweenType.DORotate:
                        {
                            var transform = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).eulerAngles
                                : (Vector3)ToValue;
                            var startValue = UseFromValue
                                ? (Vector3)FromValue
                    ...
                    case DOTweenType.DOLocalRotate:
                        {
                            var transform = Target as Transform;
                            var targetValue = UseToTarget
                                ? ((Transform)ToTarget).localEulerAngles
                                : (Vector3)ToValue;
                            var startValue = UseFromValue
                                ? (Vector3)FromValue
                    ...
                    case DOTweenType.DOAnchorPos:
                        {
                            ...
                            var targetValue = UseToTarget
                                ? ((RectTransform)ToTarget).anchoredPosition
                                : (Vector2)ToValue;
                            var startValue = UseFromValue
                                ? (Vector2)FromValue
                    ...
                    case DOTweenType.DOAnchorPosX:
                        {
                            var rectTransform = Target as RectTransform;
                            var targetValue = UseToTarget
                                ? ((RectTransform)ToTarget).anchoredPosition.x
                                : ToValue.x;
                            var startValue = UseFromValue
                                ? FromValue.x
                    ...
                    case DOTweenType.DOAnchorPosY:
                        {
                            var rectTransform = Target as RectTransform;
                            var targetValue = UseToTarget
                                ? ((RectTransform)ToTarget).anchoredPosition.y
                                : ToValue.x;
                            var startValue = UseFromValue
                                ? FromValue.x
                    ...
                    case DOTweenType.DOAnchorPosZ:
                        {
                            var rectTransform = Target as RectTransform;
                            var targetValue = UseToTarget
                                ? ((RectTransform)ToTarget).anchoredPosition3D.z
                                : ToValue.x;
                            var startValue = UseFromValue
                                ? FromValue.x
                    ...
                    case DOTweenType.DOAnchorPos3D:
                        {
                            var rectTransform = Target as RectTransform;
                            var targetValue = UseToTarget
                                ? ((RectTransform)ToTarget).anchoredPosition3D
                                : (Vector3)ToValue;
                            var startValue = UseFromValue
                                ? (Vector3)FromValue
                    ...
                    case DOTweenType.DOSizeDelta:
                        {
                            var rectTransform = Target as RectTransform;
                            var targetValue = UseToTarget
                                ? ((RectTransform)ToTarget).sizeDelta
                                : (Vector2)ToValue;
                            var startValue = UseFromValue
                                ? (Vector2)FromValue
                    ...
                    case DOTweenType.DOColor:
                        {
                            var com = Target as Graphic;
                            var targetValue = UseToTarget
                                ? ((Graphic)ToTarget).color
                                : (Color)ToValue;
                    ...
                    case DOTweenType.DOCanvasGroupFade:
                        {
                            var com = Target as CanvasGroup;
                            var targetValue = UseToTarget
                                ? ((CanvasGroup)ToTarget).alpha
                                : ToValue.x;
                    ...
                    case DOTweenType.DOFillAmount:
                        {
                            var com = Target as Image;
                            var targetValue = UseToTarget
                                ? ((Image)ToTarget).fillAmount
                                : ToValue.x;
                    ...
                    case DOTweenType.DOFlexibleSize:
                        {
                            ...
                            var targetValue = UseToTarget
                                ? (ToTarget as LayoutElement).GetFlexibleSize()
                                : (Vector2)ToValue;
                            var startValue = UseFromValue
                                ? (Vector2)FromValue
                                : com.GetFlexibleSize();
                    ...
                    case DOTweenType.DOMinSize:
                        {
                            var com = Target as LayoutElement;
                            var targetValue = UseToTarget
                                ? (ToTarget as LayoutElement).GetMinSize()
                                : (Vector2)ToValue;
                    ...
                    case DOTweenType.DOPreferredSize:
                        {
                            var com = Target as LayoutElement;
                            var targetValue = UseToTarget
                                ? (ToTarget as LayoutElement).GetPreferredSize()
                                : (Vector2)ToValue;
                            var startValue = UseFromValue
                                ? (Vector2)FromValue
