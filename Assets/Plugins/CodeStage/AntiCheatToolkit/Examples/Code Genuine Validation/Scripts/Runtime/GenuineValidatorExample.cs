...
namespace CodeStage.AntiCheat.Examples.Genuine
{
    ...
    public class GenuineValidatorExample : MonoBehaviour
    {
        ...
        private void OnGotHash(HashGeneratorResult result)
        {
            ...
            {
                status =
                    "No super secret file found, you're cheater!\n"
                    + filePath
                    + "\nSummaryHash: "
                    + result.SummaryHash;
                return;
            ...
            {
                status =
                    "Super secret file is corrupted, you're cheater!\nSummaryHash: "
                    + result.SummaryHash;
                return;
            }

            var whitelistedHashes = decrypted.Split(
                new[] { Separator },
            ...
            if (originalSummaryHash != result.SummaryHash)
            {
                ...
                for (var i = 1; i < whitelistedHashes.Length; i++)
                {
                    ...
                    {
                        status =
                            "Code hash differs, you're cheater!\nSummary hashes:\n"
                            + originalSummaryHash
                            + "\n"
                            + result.SummaryHash
                            + "\nWhitelisted hashes count: "
                            + whitelistedHashes.Length;
                        return;
                    }
                }
            }
