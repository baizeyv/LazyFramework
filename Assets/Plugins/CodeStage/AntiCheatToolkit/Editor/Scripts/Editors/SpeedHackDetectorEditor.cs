    ...
    internal class SpeedHackDetectorEditor : KeepAliveBehaviourEditor<SpeedHackDetector>
    {
        ...
        protected override bool DrawUniqueDetectorProperties()
        {
            ...
            EditorGUILayout.PropertyField(
                useDsp,
                new GUIContent(
                    ObjectNames.NicifyVariableName(nameof(SpeedHackDetector.UseDsp)),
                    useDsp.tooltip
                )
            );
#endif
            if (useDsp.boolValue)
                EditorGUILayout.HelpBox(
                    "Dsp timers may cause false positives on some hardware.\nMake sure to test on target devices before using this in production.",
                    MessageType.Warning
                );
            ...
            {
                EditorGUILayout.HelpBox(
                    "Dsp option is not available since Disable Unity Audio option is enabled.",
                    MessageType.Error
                );
                if (GUILayout.Button("Open Audio Settings"))
                {
                    ...
                    Highlighter.Highlight(
                        "Project Settings",
                        EditorTools.GetAudioManagerEnabledPropertyPath(),
...
#else
            EditorGUILayout.HelpBox(
                "Dsp option is not available since built-in Audio module is disabled.",
                MessageType.Error
            );
#endif
            return true;
