...
namespace CodeStage.AntiCheat.EditorCode.Processors
{
    ...
    {
        private const string ProGuardExclusion =
        ...
        private static void ApplyProGuardConfiguration(bool calledFromMenu)
        {
            ...
            if (!needsConfiguration)
            {
                if (calledFromMenu)
                    EditorUtility.DisplayDialog(
                        "Anti-Cheat Toolkit",
                        "ProGuard user file configuration is already applied, no action needed.",
                        "OK"
                    );
                return;
            }
            ...
            if (!calledFromMenu)
            {
                var choice = EditorUtility.DisplayDialogComplex(
                    ...
                    "You're about to make a build with minification, but there is no active ProGuard User file configured to exclude ACTk which can cause runtime exceptions.\n\n"
                        + "Would you like to apply Anti-Cheat Toolkit rules to the ProGuard User file?",
                    "Yes",
                    "No",
            ...
            if (calledFromMenu)
                EditorUtility.DisplayDialog(
                    "Anti-Cheat Toolkit",
                    $"ProGuard user file configuration at {proguardFilePath} updated.",
                    "OK"
                );
        }
