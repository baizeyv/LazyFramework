...
namespace CodeStage.AntiCheat.EditorCode
{
    ...
    public static partial class MigrateUtils
    {
        ...
        )
        {
            ...
            if (modified)
                Debug.Log(
                    $"{ACTk.LogPrefix}Migrated property {sp.displayName}:{type} at the object {label}"
                );
        ...
        internal static bool MigrateObscuredStringIfNecessary(SerializedProperty sp)
        {
            ...
            var decrypted = ObscuredString.EncryptDecryptObsolete(
                ObscuredString.GetStringObsolete(hiddenValue),
                currentCryptoKeyOld
            );

            var currentCryptoKey = ObscuredString.GenerateKey();
            var hiddenChars = ObscuredString.InternalEncryptDecrypt(
