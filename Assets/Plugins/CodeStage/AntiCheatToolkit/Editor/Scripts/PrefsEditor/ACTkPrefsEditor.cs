...
namespace CodeStage.AntiCheat.EditorCode
{
    ...
    internal partial class ACTkPrefsEditor : EditorWindow, IHasCustomMenu
    {
        ...
        {
            if (
                string.IsNullOrEmpty(newRecordKey)
            ...
            else
            {
                ...
                {
                    case 0:
                        newRecord = new PrefsRecord(
                            newRecordKey,
                            newRecordStringValue,
                            newRecordEncrypted
                        );
                        break;
                    case 1:
        ...
        private void PerformEncryptAll()
        {
            if (
                EditorUtility.DisplayDialog(
                    "Obscure ALL prefs in list?",
                    "This will apply obscuration to ALL unobscured prefs in the list.\nAre you sure you wish to do this?",
                    "Yep",
                    "Oh, no!"
                )
            )
        ...
        private void PerformDecryptAll()
        {
            if (
                EditorUtility.DisplayDialog(
                    "UnObscure ALL prefs in list?",
                    "This will remove obscuration from ALL obscured prefs in the list if possible.\nAre you sure you wish to do this?",
                    "Yep",
                    "Oh, no!"
                )
            )
        ...
        private void PerformSaveAll()
        {
            if (
                EditorUtility.DisplayDialog(
                    "Save changes to ALL prefs in list?",
                    "Are you sure you wish to save changes to ALL prefs in the list? This can't be undone!",
                    "Yep",
                    "Oh, no!"
                )
            )
        ...
        private void PerformDeleteAll()
        {
            if (
                EditorUtility.DisplayDialog(
                    "Delete ALL prefs in list?",
                    "Are you sure you wish to delete the ALL prefs in the list? This can't be undone!",
                    "Yep",
                    "Oh, no!"
                )
            )
                ...
                if (showProgress)
                {
                    if (
                        EditorUtility.DisplayCancelableProgressBar(
                            "Reading PlayerPrefs [" + (i + 1) + " of " + keysCount + "]",
                            "Reading " + keyName,
                            (float)i / keysCount
                        )
                    )
                ...
                foreach (var record in allRecords)
                {
                    if (
                        record
                            ...
                            .Contains(searchPattern.Trim().ToLowerInvariant())
                    )
                    {
        ...
        {
#if UNITY_EDITOR_WIN
            return "Software\\Unity\\UnityEditor\\"
        ...
        private string[] ReadKeysWin()
        {
            ...
            {
                Debug.LogWarning(
                    $"{ACTk.LogPrefix}Couldn't locate / access Player Prefs at path {prefsPath}. "
        ...
        private string[] ReadKeysOSX()
        {
            ...
            {
                Debug.LogWarning(
                    $"{ACTk.LogPrefix}Couldn't locate / access Player Prefs at path {plistPath}. "
                        + $"This message is harmless unless you already saved anything to the {nameof(PlayerPrefs)} or {nameof(ObscuredPrefs)}.\n"
                        + $"In such case, please report this problem to {ACTk.SupportContact} and include these data:\n"
                        + $"{ACTk.GenerateBugReport()}"
                );
