...
namespace CodeStage.AntiCheat.EditorCode
{
    ...
    internal static class ReflectionTools
    {
...
#endif
        private static readonly Type ModuleManagerType =
            ScriptingImplementationType.Assembly.GetType(
                "UnityEditor.Modules.ModuleManager",
                false
            );

        private static readonly Type ScriptingImplementationsType =
            ScriptingImplementationType.Assembly.GetType(
                "UnityEditor.Modules.IScriptingImplementations",
                false
            );
        ...
        public static bool IsScriptingImplementationSupported(
        ...
        )
        {
            if (ModuleManagerType == null)
            {
                ACTk.PrintExceptionForSupport(
                    "Couldn't find UnityEditor.Modules.ModuleManager type!"
                );
                return false;
            }
            ...
            {
                ACTk.PrintExceptionForSupport(
                    "Couldn't find UnityEditor.Modules.IScriptingImplementations type!"
                );
                return false;
            }
            ...
            {
#if UNITY_2021_2_OR_NEWER
                var mi = ModuleManagerType.GetMethod(
                ...
                {
                    ACTk.PrintExceptionForSupport(
                        "Couldn't find GetScriptingImplementations method!"
                    );
                    return false;
                }

                getScriptingImplementationsDelegate = (GetScriptingImplementations)
                    Delegate.CreateDelegate(typeof(GetScriptingImplementations), mi);
            }
            ...
            if (scriptingImplementationsTypeEnabledMethodInfo == null)
            {
                scriptingImplementationsTypeEnabledMethodInfo =
                    ScriptingImplementationsType.GetMethod(
                        "Enabled",
                ...
                {
                    ACTk.PrintExceptionForSupport(
                        "Couldn't find IScriptingImplementations.Enabled() method!"
                    );
                    return false;
                }
            }

            var enabledImplementations = (ScriptingImplementation[])
                scriptingImplementationsTypeEnabledMethodInfo.Invoke(result, null);
