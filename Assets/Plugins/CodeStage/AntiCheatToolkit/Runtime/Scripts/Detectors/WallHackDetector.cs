...
namespace CodeStage.AntiCheat.Detectors
{
    ...
    [HelpURL(
        ACTk.DocsRootUrl
            + "class_code_stage_1_1_anti_cheat_1_1_detectors_1_1_wall_hack_detector.html"
    )]
    public class WallHackDetector : ACTkDetectorBase<WallHackDetector>
    {
        ...
        /// </summary>
        [Tooltip(
            "World position of the container for service objects within 3x3x3 cube (drawn as red wire cube in scene)."
        )]
        ...
        public static WallHackDetector StartDetection()
        {
            if (Instance != null)
            {
                return Instance.StartDetectionInternal(
                    null,
                    Instance.spawnPosition,
                    Instance.maxFalsePositives
                );
            }

            Debug.LogError(
                LogPrefix
                    + "can't be started since it doesn't exists in scene or not yet initialized!"
            );
            return null;
        }
        ...
        {
            return GetOrCreateInstance.StartDetectionInternal(
                callback,
                spawnPosition,
                maxFalsePositives
            );
        ...
        private void Update()
        {
            ...
            {
                charControllerPlayer.Move(
                    new Vector3(
                        UnityEngine.Random.Range(-0.002f, 0.002f),
                        0,
                        charControllerVelocity
                    )
                );
        ...
        private WallHackDetector StartDetectionInternal(
            Action callback,
        ...
        )
        {
            ...
            {
                Debug.LogWarning(
                    $"{LogPrefix}disabled but {nameof(StartDetection)} still called from somewhere (see stack trace for this message)!",
                    this
                );
                return this;
            }
            ...
            if (callback != null && DetectorHasListeners())
            {
                Debug.LogWarning(
                    LogPrefix
                        + $"has properly configured Detection Event in the inspector or {nameof(CheatDetected)} event subscriber, but still get started with Action callback."
            ...
            {
                Debug.LogWarning(
                    $"{LogPrefix}was started without Detection Event, Callback or {nameof(CheatDetected)} event subscription."
                        + $"Cheat will not be detected until you subscribe to {nameof(CheatDetected)} event.",
                    this
                );
            }
            ...
            if (checkRaycast || checkController || checkRigidbody)
            {
                ...
                if (Physics.GetIgnoreLayerCollision(layerId, layerId))
                {
                    Debug.LogError(
                        LogPrefix
                            + "IgnoreRaycast physics layer should collide with itself to avoid false positives! See readme's troubleshooting section for details."
                    );
                }
            }
#endif
                ...
                if (checkWireframe && wfCamera == null)
                {
                    ...
                    else
                    {
                        if (!wfShader.isSupported)
                        {
                            Debug.LogWarning(
                                LogPrefix
                                    + "can't detect wireframe cheats on this platform due to lack of needed shader support!",
                                this
                            );
                            checkWireframe = false;
                        }
                        else
                        {
                            if (wfColor1 == Color.black)
                            {
                                ...
                                do wfColor2 = GenerateColor();
                                while (ColorsSimilar(wfColor1, wfColor2, 10));
                            }

                            if (shaderTexture == null)
                            {
                                shaderTexture = new Texture2D(
                                    ShaderTextureSize,
                                    ShaderTextureSize,
                                    TextureFormat.RGB24,
                                    false,
                            ...
                            {
                                var foregroundObject = GameObject.CreatePrimitive(
                                    PrimitiveType.Cube
                                );
                            ...
                            {
                                var backgroundObject = GameObject.CreatePrimitive(
                                    PrimitiveType.Quad
                                );
        ...
        private void StartRigidModule()
        {
            ...
            if (rigidPlayer.transform.localPosition.z <= 1f && rigidbodyDetections > 0)
            {
                ...
                Debug.Log(
                    LogPrefix + "rigidbody success shot! False positives counter reset.",
                    this
                );
#endif
                rigidbodyDetections = 0;
...
#if ACTK_DEBUG_ENABLED
                Debug.Log(
                    LogPrefix + "controller success shot! False positives counter reset.",
                    this
                );
#endif
                controllerDetections = 0;
        ...
        private IEnumerator CaptureFrame()
        {
#if WALLHACK_DEBUG
            if (thinWall != null)
                thinWall.GetComponent<Renderer>().enabled = false;
            if (solidWall != null)
                solidWall.GetComponent<Renderer>().enabled = false;
            if (charControllerPlayer != null)
                charControllerPlayer.GetComponent<Renderer>().enabled = false;
            if (rigidPlayer != null)
                rigidPlayer.GetComponent<Renderer>().enabled = false;
#endif
            ...
            targetTexture.ReadPixels(
                new Rect(0, 0, RenderTextureSize, RenderTextureSize),
                0,
                0,
                false
            );
...
#if WALLHACK_DEBUG
            if (thinWall != null)
                thinWall.GetComponent<Renderer>().enabled = true;
            if (solidWall != null)
                solidWall.GetComponent<Renderer>().enabled = true;
            if (charControllerPlayer != null)
                charControllerPlayer.GetComponent<Renderer>().enabled = true;
            if (rigidPlayer != null)
                rigidPlayer.GetComponent<Renderer>().enabled = true;
#endif
            ...
            var detected = diff1 || diff2 || diff3 || diff4 || diff5 || diff6 || diff7 || diff8;
...
#if ACTK_DEBUG_ENABLED
                    Debug.Log(
                        LogPrefix + "wireframe success shot! False positives counter reset.",
                        this
                    );
#endif
                    wireframeDetections = 0;
            ...
            else
            {
                ...
                Debug.LogWarning(
                    LogPrefix
                        + "wireframe wallhack detected. Details below:\n"
                        + "wfColor1: "
                        + (Color32)wfColor1
                        + ", wfColor2: "
                        + (Color32)wfColor2
        ...
        private void ShootRaycastModule()
        {
            if (
                Physics.RaycastNonAlloc(
                    serviceContainer.transform.position,
                    serviceContainer.transform.TransformDirection(Vector3.forward),
                    rayHits,
                    1.5f,
                    raycastMask
                ) > 0
...
#if ACTK_DEBUG_ENABLED
                    Debug.Log(
                        LogPrefix + "raycast success shot! False positives counter reset.",
                        this
                    );
#endif
                    raycastDetections = 0;
        ...
        private bool Detect()
        {
            ...
            if (
                controllerDetections > maxFalsePositives
