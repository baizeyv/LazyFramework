...
namespace CodeStage.AntiCheat.Detectors
{
    ...
    [HelpURL(
        ACTk.DocsRootUrl
            + "class_code_stage_1_1_anti_cheat_1_1_detectors_1_1_injection_detector.html"
    )]
    public class InjectionDetector : ACTkDetectorBase<InjectionDetector>
    {
        ...
        private InjectionDetector StartDetectionInternal(InjectionDetectedEventHandler callback)
        {
            ...
            if (callback == null && !DetectorHasListeners())
            {
                Debug.LogError(
                    $"{LogPrefix}was started without Detection Event, Callback or {nameof(CheatDetected)} event subscription."
                        + "First cheat check is executed without listeners, possible cheat will be ignored!",
                    this
                );
            }
            ...
            if (signaturesAreMissing)
            {
                TriggerCheatDetection(
                    "signatures are missing (did you enable " + ComponentName + "?)"
                );
            }
...
#if ACTK_DEBUG_NORMAL
            Debug.Log(
                ACTk.LogPrefix + "New assembly loaded: " + args.LoadedAssembly.FullName,
                this
            );
#endif
...
#if ACTK_DEBUG_NORMAL
                Debug.Log(
                    ACTk.LogPrefix + "Injected Assembly found:\n" + args.LoadedAssembly.FullName,
                    this
                );
#endif
        ...
        private bool FindInjectionInCurrentAssemblies(out string cause)
        {
            ...
            if (assembliesInCurrentDomain.Length == 0)
            {
                ...
                stopwatch.Stop();
                Debug.Log(
                    ACTk.LogPrefix + "0 assemblies in current domain! Not genuine behavior.",
                    this
                );
                stopwatch.Start();
#endif
            ...
            Debug.Log(
                ACTk.LogPrefix
                    + "Loaded assemblies scan duration: "
        ...
        private void LoadAndParseAllowedAssemblies()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                sw.Stop();
                Debug.Log(
                    ACTk.LogPrefix + "Line: " + new string(line) + ", length: " + lineLength,
                    this
                );
                sw.Start();
#endif
            ...
            Debug.Log(
                ACTk.LogPrefix
                    + "Allowed Assemblies parsing duration: "
        ...
        {
            Debug.Log(
                LogPrefix + "is not supported on current platform! This message is harmless."
            );
            return null;
        }
