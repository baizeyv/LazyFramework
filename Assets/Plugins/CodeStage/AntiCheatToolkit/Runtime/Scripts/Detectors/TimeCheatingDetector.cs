...
namespace CodeStage.AntiCheat.Detectors
{
    ...
    public class TimeCheatingDetector : ACTkDetectorBase<TimeCheatingDetector>
    {
        ...
        [Tooltip(
            "Absolute URL which will return correct datetime in response headers (you may use popular web servers like google.com, microsoft.com etc.)."
        )]
        [SerializeField]
        ...
        /// </summary>
        [Tooltip(
            "Method to use for url request. Use Head method if possible and fall back to get if server does not "
        ...
        private readonly string onlineOfflineDifferencePrefsKey = Base64Utils.ToBase64(
            ObscuredString.Encrypt("onlineOfflineSecondsDifference", "TeslaOnMars".ToCharArray())
        );
        ...
        public static TimeCheatingDetector StartDetection(
        ...
        public static Task<OnlineTimeResult> GetOnlineTimeTask(
            string url,
            RequestMethod method = RequestMethod.Head
        )
        {
        ...
        /// <param name="method">Method to use for url request. Use Head method if possible and fall back to get if server does not reply or block head requests.</param>
        /// <returns>OnlineTimeResult with UTC seconds or error.</returns>
        public static async Task<OnlineTimeResult> GetOnlineTimeTask(
            Uri uri,
            System.Threading.CancellationToken cancellationToken,
            RequestMethod method = RequestMethod.Head
        )
        {
            ...
            while (gettingOnlineTime)
            {
                ...
                {
                    return new OnlineTimeResult().SetError(
                        "Operation cancelled while waiting for previous attempt."
                    );
                }
            }
        ...
        private static void FillCurrentDomainAndUriIfNecessary()
        {
            ...
            {
                Debug.LogWarning(
                    $"{LogPrefix} Couldn't get valid string from Application.absoluteURL"
                );
                return;
            }
            ...
            if (method == RequestMethod.Head && Application.platform == RuntimePlatform.Android)
            {
                ...
                catch (Exception e)
                {
                    ACTk.PrintExceptionForSupport(
                        "Couldn't get SDK version or set Accept-Encoding header.",
                        LogPrefix,
                        e
                    );
                }
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(dateHeader))
                {
                    ...
                    if (success)
                    {
                        ...
                        var onlineSecondsUtc =
                            onlineTimeUtc.Ticks / (double)TimeSpan.TicksPerSecond;
                        result.SetTime(onlineSecondsUtc, onlineTimeUtc);
                    }
                    else
                    {
                        result.SetError(
                            "Couldn't parse 'Date' response header value\n " + dateHeader,
                ...
                {
                    result.SetError(
                        "Couldn't find 'Date' response header value!",
                        request.responseCode
                    );
                }
            }
        ...
        public bool IsReadyForForceCheck()
        {
            ...
            if (IsCheckingForCheat)
            {
                Debug.LogWarning(
                    LogPrefix
                        + "Can't force cheating check since another check is already in progress."
                );
        ...
        private TimeCheatingDetector StartDetectionInternal(
            float checkInterval,
            TimeCheatingDetectorEventHandler cheatCheckedEventHandler = null
        )
        {
        ...
        private IEnumerator CheckForCheat()
        {
            ...
            var lastOfflineOnlineDifference = PlayerPrefs.GetInt(
                onlineOfflineDifferencePrefsKey,
                0
            );
            if (lastOfflineOnlineDifference != 0)
            {
                lastOfflineOnlineDifference ^= int.MaxValue;
                var differenceOfDifferences = Math.Abs(
                    offlineOnlineDifference - lastOfflineOnlineDifference
                );

                if (realCheatThreshold < 10)
                    Debug.LogWarning(
                        LogPrefix
                            + "Please consider increasing realCheatThreshold to reduce false positives chance!"
                    );
                    ...
                    if (LastResult == CheckResult.WrongTimeDetected || !ignoreSetCorrectTime)
                    {
                        ...
                        Debug.LogWarning(
                            LogPrefix
                                ...
                                + offlineSecondsUtc
                                + "\n"
                                + "lastOnlineSecondsUtc: "
                                + lastOnlineSecondsUtc
                                + "\n"
                                + "offlineOnlineDifference: "
                                + offlineOnlineDifference
                                + "\n"
                                + "lastOfflineOnlineDifference: "
                                + lastOfflineOnlineDifference
                                + "\n"
            ...
            PlayerPrefs.SetInt(
                onlineOfflineDifferencePrefsKey,
                offlineOnlineDifference ^ int.MaxValue
            );
            IsCheckingForCheat = false;
            ReportCheckResult();
        }
...
#pragma warning restore 67
        ...
        public static void StartDetection(
            ...
            Action<ErrorKind> errorCallback,
            int interval
        ) { }
        ...
        private const string ErrorMessage =
            LogPrefix
            + " is disabled with ACTK_PREVENT_INTERNET_PERMISSION conditional or is not supported on current platform!";
