...
namespace CodeStage.AntiCheat.Detectors
{
    ...
    [HelpURL(
        ACTk.DocsRootUrl
            + "class_code_stage_1_1_anti_cheat_1_1_detectors_1_1_speed_hack_detector.html"
    )]
    public class SpeedHackDetector : ACTkDetectorBase<SpeedHackDetector>
    {
        ...
        [field:
            SerializeField,
            Tooltip(
                "Uses AudioSettings.dspTime under the hood, which can catch some extra speed hacks "
        ...
        private void Update()
        {
            ...
            var cheatedEnvironment = IsTicksCheated(
                TimeUtils.GetEnvironmentTicks(),
                ref previousEnvironmentTicks,
                reliableDelta
            );
            var cheatedRealtime = IsTicksCheated(
                TimeUtils.GetRealtimeTicks(),
                ref previousRealtimeTicks,
                reliableDelta
            );
            ...
            if (cheatedEnvironment || cheatedRealtime || cheatedDsp)
            {
                ...
                else
                {
                    ...
                    Debug.LogWarning(
                        LogPrefix
                            + "detection! Allowed false positives left: "
                            + (maxFalsePositives - currentFalsePositives),
                        this
                    );
#endif
            ...
            else if (currentFalsePositives > 0 && coolDown > 0)
            {
                ...
                Debug.Log(
                    LogPrefix
                        + "success shot! Shots till cool down: "
                        + (coolDown - currentCooldownShots),
                    this
                );
#endif
        ...
        private SpeedHackDetector StartDetectionInternal(
            Action callback,
            float checkInterval,
            byte falsePositives,
        ...
        )
        {
            ...
            if (callback != null && DetectorHasListeners())
                Debug.LogWarning(
                    $"{LogPrefix}has properly configured Detection Event in the inspector or {nameof(CheatDetected)} event subscriber, but still get started with Action callback."
